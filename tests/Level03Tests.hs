...
unitTests = do
  ...
  hspec . with app' $ do
      ...
      describe "Add Route" $ do
        it "Should respond with Error when given empty comment" $
      ...
      describe "View Route" $ do
        it "Should respond correctly when given a topic " $
      ...
      -- 3) The '<topic>/view' route will respond with an error when given an empty topic
        it "Should respond with Error when given an empty topic" $
          get "//view" `shouldRespondWith` "Empty Topic" {matchStatus = 400}
      -- 4) A gibberish route will return a 404

      describe "Bad Route" $ do
        it "Gibberish route returns a 404" $
          get "/eou/thh/gg" `shouldRespondWith` "Unknown Route" {matchStatus = 404}
